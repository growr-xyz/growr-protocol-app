  ...
  if (this.options.customButtons) {
    ...
    customButtonsElArray.map((el, index) => {
      el.addEventListener("click", (event) =>
        _this2.options.customButtons[index].onClick(event)
      );
    });
  }
};
...
Toast.prototype._open = function () {
  ...
  let customButtons = "";
  if (this.options.customButtons) {
    customButtons = this.options.customButtons.map(
      (customButton) =>
        `<button type="button" class="toastjs-btn toastjs-btn--custom">${customButton.text}</button>`
    );
